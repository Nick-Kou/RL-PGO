...
/* ************************************************************************** */
Eigen::VectorXd ReprojectionBundlerFactor::error(const Variables& values) const {
    ...
    Eigen::VectorXd ReprojectionBundlerFactor::getter() {
        Eigen::VectorXd v(0);
    return v;}


/* ********************************************************************************** */
//    Eigen::VectorXd ReprojectionBundlerFactor::error(const Variables& values)  {
//        const Sophus::SE3d& pose = values.at<Sophus::SE3d>(keys()[0]);
//        const CalibBundler& calib = values.at<CalibBundler>(keys()[1]);
//        const Eigen::Vector3d& land = values.at<Eigen::Vector3d>(keys()[2]);
//        return projectBundler(pose, calib, land) - p_measured_;
