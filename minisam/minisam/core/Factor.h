...
class Factor {
 ...
 virtual Eigen::VectorXd getter() = 0;
  ...
  const std::vector<Key>& keys() const { return keylist_; }
     std::vector<Key>& keys_pub()  { return keylist_; }
  ...
  //virtual Eigen::VectorXd error(const Variables& variables) = 0 ;
  ...
  // whiten error
  virtual Eigen::VectorXd weightedError(const Variables& variables)  ;
