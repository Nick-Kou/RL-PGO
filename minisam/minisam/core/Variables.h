...
class Variables {
 ...
 public:
  ...
  // increasing variable ordering from smallest to lowest
  VariableOrdering increasing_VariableOrdering() const;
  ...
  Variables retract(const Eigen::VectorXd& delta,
    ...
    // retract, input size should be dim
    Variables retract_rot_2D(const Eigen::VectorXd& delta,
                      const VariableOrdering& variable_ordering) const;
