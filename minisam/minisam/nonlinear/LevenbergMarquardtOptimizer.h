...
int lambda_iteration_;
size_t iterations_; 


//uncomment below if using the previous methods with every method separated
   double hessian_diag_max = 0;
   double hessian_diag_max_sqrt = 0;
...
int optimize_initial(
      const FactorGraph& graph, const Variables& init_values,
      Variables& opt_values, const VariablesToEliminate& var_elimiated =
                                 VariablesToEliminate());

NonlinearOptimizationStatus tryLambda_new(
 ...
 NonlinearOptimizationStatus iterate_after_init_optimize(const FactorGraph& graph,
                                      Variables& result_values);
                                     

NonlinearOptimizationStatus iterate_try_andopt(const FactorGraph& graph, Variables& values); 


NonlinearOptimizationStatus tryLambda_new2
(Eigen::SparseMatrix<double>& A, Eigen::VectorXd& b,
    const Eigen::VectorXd& g, const Eigen::VectorXd& hessian_diag,
...
NonlinearOptimizationStatus final_optimize
