...
#define WRAP_TYPE_PRIOR_FACTOR(T, TYPENAME) \
  py::class_<PriorFactor<T>, Factor, std::shared_ptr<PriorFactor<T>>>(m, \
    ...
    m.def("getter_diff_", [](PriorFactor<T> &obj) { \
        return obj.getter_diff_(); \
      });
// Note for getter diff function implementation above does not work, it seems that you need to define the type and can't just use T: https://stackoverflow.com/questions/47487888/pybind11-template-class-of-many-types
...
void wrap_slam(py::module& m) {
  ...
  // slam util
  m.def("get_so2_meas", &get_so2_meas);
  m.def("get_rotation_features", &get_rotation_features);
  m.def("Standardize_Pose_Graph", &Standardize_Pose_Graph);
  m.def("ATE_minisam", &ATE_minisam);
  m.def("ARE_minisam", &ARE_minisam);
  m.def("Trans_2D_Solver", &Trans_2D_Solver);
  m.def("writeG2O", &writeG2O);
  m.def("writeTORO", &writeTORO);
