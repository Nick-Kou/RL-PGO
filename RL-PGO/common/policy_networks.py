...
class PolicyNetwork(nn.Module):
    ...
    def evaluate(self, state, epsilon=1e-6):
        ...
        #action_range_torch = torch.from_numpy(self.action_range)
        #action_range_torch = action_range_torch.cuda()
        #action = action_range_torch * action_0
        action = self.action_range * action_0

        #log = np.log(self.action_range)
        ...
        #debug_1=Normal(mean, std).log_prob(mean + std * z.to(device)) - torch.log(1. - action_0.pow(2) + epsilon)
        #log_prob = Normal(mean, std).log_prob(mean+ std*z.cuda) - torch.log(1. - action_0.pow(2) + epsilon) -  log
    ...
    def get_action(self, state, deterministic):
        ...
        #action_range_torch = action_range_torch.cuda()
        #debug
        # de=torch.tanh(mean + std * z)
        # de1=action_range_torch * de
        #End debug
...
##################### My implementation with new action range and log in evluation() comment this and uncomment the implementation above for original implementations #################
class SAC_PolicyNetworkLSTM(PolicyNetworkBase):
    def __init__(self, state_space, action_space, hidden_size, action_range, init_w=3e-3, log_std_min=-20,log_std_max=2):
        super().__init__(state_space, action_space, action_range=action_range)
        ...
        # Kaiming normal (try uniform as well) initialization
        nn.init.kaiming_normal_(self.linear1.weight, nonlinearity='relu')
        nn.init.kaiming_normal_(self.linear2.weight, nonlinearity='relu')
        nn.init.kaiming_normal_(self.linear3.weight, nonlinearity='relu')
        nn.init.kaiming_normal_(self.linear4.weight, nonlinearity='relu')

        # Xavier uniform (try normal as well)
    ...
    def get_action(self, state, last_action, hidden_in, deterministic=True):
        ...
        action_range_torch = torch.from_numpy(self.action_range)
        action_range_torch = action_range_torch.cuda()
        action = action_range_torch * torch.tanh(mean + std * z)
        if deterministic:
            action = action_range_torch * torch.tanh(mean)
            action = action.detach().cpu().numpy()
        else:
            action = action.detach().cpu().numpy()
        return action[0][0], hidden_out

######### Rotation SACLSTM (is the exact same as the original commented 2 classes above this but do not currently want to change the name of all the training files that use the original name
#SAC_PolicyNetowrkLSTM)##################
class SAC_PolicyNetworkLSTM_rotation(PolicyNetworkBase):
    def __init__(self, state_space, action_space, hidden_size, action_range=1., init_w=3e-3, log_std_min=-20, log_std_max=2):
        super().__init__(state_space, action_space, action_range=action_range)
    ...
    def forward(self, state, last_action, hidden_in):
        ...
        last_action = last_action.permute(1,0,2)
        # branch 1
        #1 1 512 for single step graph
        #46,2,512 for batch(seqlen,batch,hidden)
        fc_branch = F.relu(self.linear1(state))
        # branch 2
        #1 1 10
        #1 1 43 for single step graph
        #46,2,43 for batch (seqlen,batch,state dim+actiondim)
        lstm_branch = torch.cat([state, last_action], -1)
        #1 1 512 for single step graph
        #46,2,512 for batch 43 turn into 512 after passing the linear layer
        lstm_branch = F.relu(self.linear2(lstm_branch))
        #1 1 512    ,  tuple2 of 1 1 512 for single step graph
        #46,2,512      tuple2  of 1,2,512 for batch
        lstm_branch, lstm_hidden = self.lstm1(lstm_branch, hidden_in)  # no activation after lstm
        # merged
        #1 1 1024 for single step graph
        #46,2,1024 for batch graph
        merged_branch=torch.cat([fc_branch, lstm_branch], -1)
        #1 1 512 for single step graph
        #46,2,512 for batch graph
        x = F.relu(self.linear3(merged_branch))
        #1 1 512 for single step
        #46,2,512 for batch graph
        x = F.relu(self.linear4(x))
        #1 1 512 for single step
        #2,46,512 batch,seqleng,hidden after permute
        x = x.permute(1,0,2)  # permute back
        #1 1 2 for single step
        #2,46,2 batch,seqlength,dim of action
        mean    = self.mean_linear(x)
        # mean    = F.leaky_relu(self.mean_linear(x))
        #1 1 2 for single step
        #2,46,2 batch,seqlength,dim of action
    ...
    def evaluate(self, state, last_action, hidden_in, epsilon=1e-6):
        ...
        #log_prob = Normal(mean, std).log_prob(mean + std * z.cuda()) - torch.log(1. - action_0.pow(2) + epsilon) - np.log(self.action_range)
        log_prob = Normal(mean, std).log_prob(mean + std * z.cuda()) - torch.log(
            1. - action_0.pow(2) + epsilon) - np.log(1.0)
    ...
    def get_action(self, state, last_action, hidden_in,device, deterministic=True):
        ...
        action = torch.tanh(mean).detach().cpu().numpy() if deterministic else \
            action.detach().cpu().numpy()

        return action[0][0], hidden_out


class SAC_PolicyNetwork_NO_LSTM_rotation(PolicyNetworkBase):
